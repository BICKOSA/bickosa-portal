...
import { CampaignUpdatesPanel } from "@/app/(portal)/donate/[slug]/_components/campaign-updates-panel";
...
export default async function AdminCampaignUpdatesPage({ params }: CampaignUpdatesPageProps) {
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-[var(--text-2)]">
            Post updates for <span className="font-semibold text-[var(--text-1)]">{campaign.title}</span>.
          </p>
        </div>
        <Button asChild variant="outline">
          <Link href={`/admin/campaigns/${campaign.id}/edit`}>Back to Campaign</Link>
        </Button>
      </div>
