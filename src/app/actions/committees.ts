...
async function requireAuthedMember() {
  ...
  if (!session) {
    return { error: { ok: false, message: "You must be signed in." } as const, session: null };
  }

  const isVerified = await getUserVerifiedMemberState(session.user.id, Boolean(session.user.emailVerified));
  if (!isVerified) {
    return {
      error: { ok: false, message: "Only verified members can submit committee nominations." } as const,
      session: null,
    };
  }
...
async function sendNominationRequestEmail(input: {
  ...
  const responseUrl = getCommitteeResponseUrl(input.committeeId, input.nominationId);
  await sendEmail({
    to: input.nomineeEmail,
    subject: `You've been nominated for the ${input.committeeName}`,
    react: createElement("div", null, [
      createElement("p", { key: "greeting" }, `Hello ${input.nomineeName || "Member"},`),
      createElement(
        "p",
        { key: "line1" },
        `${input.nominatedByName} nominated you to join the ${input.committeeName}.`,
      ),
      createElement("p", { key: "line2" }, `Committee mandate: ${input.committeePurpose}`),
      createElement("p", { key: "line3" }, `Accept or decline here: ${responseUrl}`),
    ]),
    text: `${input.nominatedByName} nominated you for ${input.committeeName}. Mandate: ${input.committeePurpose}. Respond: ${responseUrl}`,
  });
}

export async function submitSelfNomination(committeeId: string, reason: string): Promise<ActionResult> {
  ...
  if (
    ...
    return { ok: false, message: "Nominations are currently closed for this committee." };
  }
  ...
  const [row] = await db
    ...
    .onConflictDoNothing({
      target: [committeeNominations.committeeId, committeeNominations.nomineeId],
    })
    .returning({ id: committeeNominations.id });

  if (!row) {
    return { ok: false, message: "A nomination already exists for this committee and nominee." };
  }
...
export async function submitPeerNomination(
  ...
  const parsed = peerNominationSchema.safeParse({ committeeId, nomineeId, reason });
  if (!parsed.success) {
    return { ok: false, message: parsed.error.issues[0]?.message ?? "Invalid nomination input." };
  }
  ...
  if (!row) {
    return { ok: false, message: "This member already has a nomination for this committee." };
  }
  ...
  return { ok: true, message: "Nomination submitted and nominee notified by email." };
}
...
async function transitionNominationStatus(params: {
  ...
  if (params.nextStatus === "confirmed_willing" || params.nextStatus === "declined") {
...
export async function acceptNomination(
  ...
  if (!parsedNote.success) {
    return { ok: false, message: parsedNote.error.issues[0]?.message ?? "Invalid response note." };
  }
  ...
  if (nomination.nomineeId !== session.user.id) {
    return { ok: false, message: "You can only respond to your own nomination." };
  }
  ...
  const adminUserIds = await listAdminUserIdsForCommitteeNotifications(session.user.id);
  ...
  revalidatePath(`/committees/${nomination.committeeId}/respond/${nomination.id}`);
  revalidatePath(`/admin/committees/${nomination.committeeId}/nominations`);

  return { ok: true, message: "Nomination accepted. Thank you for your willingness to serve." };
}
...
export async function appointToCommittee(
  ...
  if (nomination.status !== "confirmed_willing") {
    return { ok: false, message: "Only confirmed willing nominees can be appointed." };
  }
  ...
  if (committee.maxMembers !== null && currentCount >= committee.maxMembers) {
    return { ok: false, message: "This committee is already at its max members target." };
  }
  ...
  if (nominee) {
    ...
    await sendEmail({
      ...
      react: createElement("div", null, [
        createElement("p", { key: "greet" }, `Hello ${nominee.name || "Member"},`),
        createElement(
          "p",
          { key: "body" },
...
export async function sendNominationReminder(nominationId: string): Promise<ActionResult> {
  const session = await getSession();
  if (!session || !isAdminUserRole((session.user as { role?: string }).role)) {
  ...
  if (nomination.status !== "pending") {
    return { ok: false, message: "Only pending nominations can receive reminders." };
  }
...
export async function createCommitteeAction(input: {
  ...
  if (!parsed.success) {
    return { ok: false, message: parsed.error.issues[0]?.message ?? "Invalid committee input." };
  }
...
export async function advanceCommitteeStatusAction(committeeId: string): Promise<ActionResult> {
  const session = await getSession();
  if (!session || !isAdminUserRole((session.user as { role?: string }).role)) {
  ...
  if (nextStatus === "nominations_open") {
    const recipients = await db
      .select({ userId: alumniProfiles.userId })
      .from(alumniProfiles)
      .where(eq(alumniProfiles.verificationStatus, "verified"));
  ...
  revalidatePath("/committees");
  revalidatePath(`/committees/${committee.id}`);
  return { ok: true, message: "Committee status advanced." };
}
